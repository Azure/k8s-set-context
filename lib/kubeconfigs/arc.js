...
function getArcKubeconfig() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield runAzCliCommand(azPath, "account show");
        try {
            yield runAzCliCommand(azPath, "extension remove -n connectedk8s");
        }
        catch (_a) {
            // expected when it is the first time running the action
            core.debug("Failed to remove connectedk8s");
        }
        yield runAzCliCommand(azPath, "extension add -n konnectedk8s");
        yield runAzCliCommand(azPath, "extension list");
